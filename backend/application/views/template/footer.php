<div class="show-prod" style="display:none;position:absolute;padding:20px;border:1px solid #fff;background-color:rgb(0, 175, 233);z-index:999;margin-left:300px;margin-top:150px;border-radius:5px;">
  <span class="name-product" style="font-weight:bold;color:#fff;float:left;margin:10px;"></span>
  <span class="close" style="color:#fff;cursor:pointer;float:right;font-weight:bold;margin:10px;">Close (x)</span><br>
  <img class="zoom-prod" src="" alt="" style="border-radius:5px;width:466px;height:466px;">
</div>
...
function add_subcategory(id)   {
  var api_img = 'http://localhost/serviceadmin/uploads/category/';
  $.ajax({
...
function get_banner_by_id(id)   {
  save_method = 'update';
  api_img = 'http://localhost:7777/serviceadmin/uploads/banner/';
  $.ajax({
...
function get_product_by_id(id)   {
  ...
  $.ajax({
    ...
    dataType : "JSON",
    success: function(data)  {
      // console.log(data[0].product_condition);
        ...
        $('input[name="condision_edit"]').filter("[value=" +data[0].product_condition +"]").attr('checked', true);
        $('[name="weight"]').val(data[0].weight);
        $('[name="brand"]').val(data[0].brand);
        $('[name="discount"]').val(data[0].discount);
        $('[name="stock"]').val(data[0].stock);
        $('[name="product_variant_id"]').val(data[0].product_variant_id);
